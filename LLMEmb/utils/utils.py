...
def metric_report(data_rank, topks=(5, 10, 20)):
    """Compute Recall@K and NDCG@K for multiple K.

    In single-positive next-item recommendation, Recall@K == HR@K.
    Both keys are emitted for backward compatibility (HR@K after Recall@K and NDCG@K).

    Args:
        data_rank: np.array of predicted ranks (0-based) for the positive item.
        topks: iterable of K values.

    Returns:
        Ordered dict with keys: Recall@K, NDCG@K, HR@K for each K in topks.
    """
    n = len(data_rank)
    res = {}
    ...
    for topk in topks:
        ...
        res["Recall@{}".format(topk)] = recall_val
        res["NDCG@{}".format(topk)] = ndcg_val
        res["HR@{}".format(topk)] = recall_val   # identical to Recall@K

...
class SeqEvalDatasetFixedNeg(Dataset):
    """Evaluation dataset with fixed negative samples (no random sampling).

    Mirrors the output format of SeqDataset, but uses pre-specified
    positive and negative items instead of sampling randomly.
    """

    def __init__(self, eval_data, user_ids, item_num, max_len, fixed_neg, fixed_pos):

        super().__init__()
        self.data = eval_data          # list of history sequences (train for valid, train+valid for test)
        self.user_ids = user_ids       # list of actual user IDs, aligned with eval_data
        self.item_num = item_num
        self.max_len = max_len
        self.fixed_neg = fixed_neg     # dict[user_id] = list[int]
        self.fixed_pos = fixed_pos     # dict[user_id] = int
        self.var_name = ["seq", "pos", "neg", "positions"]

        # MANDATORY stdout diagnostics
        first_uid = user_ids[0] if user_ids else -1
        first_seq = eval_data[0] if eval_data else []
        first_pos = fixed_pos.get(first_uid, -1)
        first_neg = fixed_neg.get(first_uid, [])
        print(
            "[SeqEvalDatasetFixedNeg] dataset_len={} max_len={} first_user={} first_pos={} first_neg_len={} first_seq_tail10={}".format(
                len(self.data), max_len, first_uid, first_pos, len(first_neg), first_seq[-10:]
            ),
            flush=True,
        )
    ...
    def __getitem__(self, index):
        ...
        inter = self.data[index]             # history items (does NOT include the target)
        user_id = self.user_ids[index]

        pos = self.fixed_pos[user_id]
        neg = np.array(self.fixed_neg[user_id], dtype=np.int32)

        # Build seq from history
        seq = np.zeros([self.max_len], dtype=np.int32)
        idx = self.max_len - 1

        ...
        for batch in tqdm(test_loader, desc=desc):
            ...
            with torch.no_grad():
                ...
                if not _first_batch_printed:
                    print(
                        "[EVAL_BATCH] seq_shape={} pos_shape={} neg_shape={} candidate_shape={}".format(
                            tuple(inputs["seq"].shape),
                            tuple(inputs["pos"].shape),
                            tuple(inputs["neg"].shape),
                            tuple(inputs["item_indices"].shape),
                        ),
                        flush=True,
                    )
